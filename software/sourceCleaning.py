...
os.chdir('/Users/alexgagliano/Documents/Research/Transient_ML/scripts/')
...
def removeDuplicateSN_hostTable(host_DF, severalHosts):
    for host in severalHosts:
        ...
        if (sep[idx].arcsec <= 1) and (row.TransientDiscoveryYear == host_DF_sub.loc[idx_DF, 'TransientDiscoveryYear']):
            ...
            #print(index)
            print("Matched:")
            print("%s" % row['Transient Name'])
            print("%s" % row.TransientClass)
            print("%s "%host_DF_sub.loc[idx_DF, 'Transient Name'])
...
def makeCuts(df,cuts=[],dict=""):
    ####### CHECKPOINT 02/02/2020 (Happy palindrome day :) )
    for cut in cuts:
        if cut == "n":
