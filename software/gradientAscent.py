...
def plot_DLR_vectors_GD(size, path, transient, transient_df, host_dict_candidates, host_dict_final, host_df, R_dict, ra_dict, df = "TNS", dual_axes=0, scale=1, postCut=0):
    ...
    tempRA = Angle(row.RA_deg, unit=u.degree)
    tempDEC = Angle(row.DEC_deg, unit=u.degree)
...
def get_clean_img(ra, dec, px, band):
    os.chdir('/Users/alexgagliano/Documents/Research/Transient_ML/fits')
    #first, mask the data
    a = find_all("PS1_ra={}_dec={}_{}arcsec_{}.fits".format(ra, dec, int(px*0.25), band), '.')
    if not a:
        get_PS1_Pic(ra, dec, px, band)
        a = find_all("PS1_ra={}_dec={}_{}arcsec_{}.fits".format(ra, dec, int(px*0.25), band), '.')
...
def gradientDescent(SN_dict, SN_dict_postDLR, SN_names, hostDF, transientDF, fn, plot=1):
    ...
    #    if True:
            ...
            ra = transientDF.loc[transientDF['Name'] == transient_name, 'RA_deg'].values[0]
            dec = transientDF.loc[transientDF['Name'] == transient_name, 'DEC_deg'].values[0]
            px = 800
            os.chdir('/Users/alexgagliano/Documents/Research/Transient_ML/fits')
            ...
            for band in ['g', 'r', 'i']:
                #os.remove("PS1_ra={}_dec={}_{}arcsec_{}_wt.fits".format(ra, dec, int(px*0.25), band))
            ...
            os.chdir('/Users/alexgagliano/Documents/Research/Transient_ML/plots/quiverMaps')
            nancount = 0
            obj_interp = []
...
#                        sources[col].info.format = '%.8g'  # for consistent table output
                    ...
                    if plot:
                        ...
                        plt.axis('off')
                        plt.savefig("detectedStars_%s.png"%transient_name, bbox_inches='tight')
                        plt.close()
                except:
        ...
        #        step_sizes[int(i)] = np.max([step_sizes[int(i)], 25])
            ...
            if plot:
                ...
                axs[2].axis('off')
                plt.savefig("backgrounds_%s.png" % transient_name, bbox_inches='tight')
                plt.close()
            ...
            if plot:
                ...
                plt.axis('off')
                plt.savefig("normalizedMeanImage_%s.png" % transient_name, bbox_inches='tight')
                plt.close()
                ...
                plt.axis('off')
                plt.savefig("normalizedMeanBackground_%s.png" % transient_name, bbox_inches='tight')
                plt.close()
...
#            background  = (0.66*background/np.max(background) +  imgWeight*meanImg/np.nanmax(meanImg))*np.max(background)
            ...
            if True:
                ...
                if plot:
                    ...
                    plt.axis("off")
                    plt.savefig("fullBackground_%s.png"%transient_name, bbox_inches='tight')
                    plt.close()
                ...
                if a:
                    ...
                    if len(a) > 0:
                        ...
                        print("Dict value: %i"%SN_dict_postDLR[transient_name][0],file=f)
                        N = len(hostDF)
                ...
                if plot:
                    ...
                    plt.axis('off')
                    plt.savefig("quiverMap_%s.png"%transient_name, bbox_inches='tight')
                N_associated += 1
            #else:
            #    print("Ope! No background flux here!", file=f)
            #    fig = plt.figure(figsize=(20,20))
            #    ax = fig.gca()
            #    ax.imshow(i_img, norm=colors.PowerNorm(gamma = 0.5, vmin=1, vmax=1.e4), cmap='gray')#, cmap='gray', norm=LogNorm())
            #    plt.axis('off')
            ...
            if N_associated%10 == 0:
                ...
                with open("/Users/alexgagliano/Documents/Research/YSE_PhotometrySurvey/gals_postDLR_wGD.p", 'wb') as fp:
                    pickle.dump(SN_dict_postDLR, fp, protocol=pickle.HIGHEST_PROTOCOL)
                hostDF.to_csv("/Users/alexgagliano/Documents/Research/Transient_ML_Box/tables/hostDF_postDLR_0514_postGD_redoFinal_noHost.tar.gz", index=False)
        except:
             continue
